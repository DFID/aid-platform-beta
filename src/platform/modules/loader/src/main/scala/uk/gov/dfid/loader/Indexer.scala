...
class Indexer @Inject()(db: DefaultDB, engine: ExecutionEngine, sectors: Sectors) {
  ...
  def index {
    ...
    // loop over each project and index it
    ...
    projects.map { doc =>
      ...
      val organisations   = doc.getAs[BSONArray]("participatingOrgs").map { values =>
    ...
    for(
      ...
      stats.map { stat =>
        ...
        println("Indexing Country Suggestion")
        println(bean)
